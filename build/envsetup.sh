...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/horizon/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            lunch horizon_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop ro.horizon.device | grep -q "$HORIZON_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $HORIZON_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.horizon.device | grep -q "$HORIZON_BUILD");
    then
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                horizon|otapackage|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
...
function dopush()
{
    ...
    if (adb shell getprop ro.horizon.device | grep -q "$HORIZON_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
