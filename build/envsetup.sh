...
- githubremote:    Add git remote for HorizonDroid Github.
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/horizon/vars/horizon_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            lunch horizon_$target-$horizon_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/HorizonDroid*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.horizon.device | grep -q "$CUSTOM_BUILD"); then
...
function githubremote()
{
    ...
    git remote add github https://github.com/HorizonDroidLab/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.horizon.device | grep -q "$CUSTOM_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.horizon.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/horizon/build/tools/repopick.py $@
}
