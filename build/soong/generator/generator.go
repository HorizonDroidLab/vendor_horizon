...
func init() {
	android.RegisterModuleType("horizon_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = horizonExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = horizonExpandVariables(ctx, dep_file)
	...
	cmd := horizonExpandVariables(ctx, String(g.properties.Cmd))
